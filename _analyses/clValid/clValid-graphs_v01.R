...
get_ft_graphs <- function() {
  # Name of the file to create
  ft_fname <- paste0("graph_", feats_set_idx, dd, mm, "_", cc, "_1.png")
  # Save as PNG
  png(
  ...
  if (feats_set_idx == 1) {
    ...
    column_names <- feats_set[[feats_set_idx]][1:24]
    ...
    par(cex=0.7, mai=c(0.8,0.5,0.05,0.05))
    column_names <- feats_set[[feats_set_idx]][25]
...
# # Dataset loop
# foreach::foreach(dd = 1:4) %:%
#   foreach::foreach(mm = c(1:3, 5:9)) %:%
#     foreach::foreach (cc = cluster_number:1, .inorder = FALSE, errorhandling = "remove") %do% {
for (dd in 4) { # 1:4
  for (mm in 7:9) { # c(1:3, 5:9)
