...
get_feature_graphs <- function() {
  ...
  if (n1 == 1) {
    ...
    par(fig=c(27/35,31/35,0,1), cex=1, new=TRUE)
    ...
    boxplot(cluster_elems, las=2, xlab=short_labels, col="darksalmon",
            ylim=c(0,1))
    # Plot kWh/year
    par(fig=c(31/35,1,0,1), cex=1, new=TRUE)
    short_label <- "kWh/year"
    cluster_elems <- w_feats[cluster_idx,]
    sum_kWh      <- subset(cluster_elems, select = sum)
    overall_days <- subset(cluster_elems, select = overall_days)
    values <- sum_kWh / overall_days * 365
    boxplot(values, las=2, xlab=short_label, col="darkorchid4",
            ylim=c(0,20000))
    # Save file
    dev.off()
  }
