...
manage_duplicates <- function(df, file_name) {
  ...
  # Readable types of data
  unreadable_types <- 
    !(df[["V1"]] %in% c("A5D", "B5D", "F5D", "P5D", "RF5D", "F1", "P1D", "P2D"))
  # Check if there's at least some "readable" type of date
  if (all(unreadable_types)) {
    return(NULL)
  } 
  # Remove all unrecognizable entries
  else if (sum(unreadable_types) > 0) {
    df <- df[-c(which(unreadable_types)), ]
  }
  ...
  dates[date_idx_B] <- as.POSIXct(df$V3[date_idx_B], tz="GMT", origin=time_origin) -
    lubridate::hours(df$V4[date_idx_B])
  dates[date_idx_C] <- as.POSIXct(df$V4[date_idx_C], tz="GMT", origin=time_origin) -
    lubridate::hours(df$V5[date_idx_C])
...
foreach (x = 1:length(fnames)) %do% {
  ...
  if (!is.null(g_df)) {
    ...
    data.table::fwrite(
      ...
      file       = paste0(goiener_report_folder, "wrong_usr2raw.csv"),
      row.names  = FALSE,
      col.names  = FALSE,
      sep        = ",",
      na         = "",
