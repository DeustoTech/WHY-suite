# Version 4 accounts for the maximum point value of all periods in the new tariff
...
for (dset in c("goi")) { #  c("goi", "iss", "lcl", "ref")
  ...
  # Filenames loop
  o <- foreach::foreach (ff = 1:length(fnames), .combine = rbind, .packages = c("lubridate")) %dopar% {
    # Complete filename
