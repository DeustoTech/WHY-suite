...
scripts <- function(script_selection) {
  ...
  if (script_selection == 23) {
    get_features_from_cooked_dataframe_test <- function(cdf, type_of_analysis) {
      browser()
    ...
    get_features_from_ext_datasets_test <- function(input_folder, output_folder, type_of_analysis, parallelize=TRUE) {
      ...
      inloop_feats <- function(x, col_names) {
        ...
        if (!edf$is_0) {
          ...
          # GET FEATURES
          ff_feats <- get_features_from_cooked_dataframe_test(
            edf, type_of_analysis)
    ...
    type_of_analysis <- "extra"
    # Compute features
    feats <- get_features_from_ext_datasets_test(
      input_folder, output_folder, type_of_analysis, parallelize=FALSE)
