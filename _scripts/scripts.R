...
scripts <- function(script_selection) {
  ...
  # SCRIPT 40
  # TRIAL TO SEE HOW TO CORRECT THE DAYLIGHT SAVING TIME
  if (script_selection == 40) {
    fname <- "G:/Mi unidad/WHY/Datasets/goi/ext/bac40e555262d1734f2dbd82ea9c7e2e.RData"
    load(fname)
    
    # m <- whyT2.1::get_heatmap_matrix(as.data.frame(fname))
    ...
    out_path <- "G:/Mi unidad/WHY/Github/why-T2.1/_scripts/out.RData"
    save(edf, file = out_path)
    # m <- whyT2.1::get_heatmap_matrix(as.data.frame(out_path))
    # whyT2.1::plot_heatmap_matrix(
    #   m,
    #   format_file="png",
